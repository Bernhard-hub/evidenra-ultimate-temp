  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900 text-white p-6">
      ...
      <div className="flex gap-3 mb-8">
        ...
        <button
          onClick={handleReset}
          className="px-6 py-4 bg-gray-800 hover:bg-gray-700 rounded-xl font-semibold shadow-lg transition-all"
          title="Reset Evolution Data"
        >
          🔄 RESET
        </button>
      </div>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          ...
          {gapesStats && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <MetricCard label="API Calls" value={gapesStats.totalCalls} icon="📡" color="cyan" />
              <MetricCard label="Success Streak" value={gapesStats.streak} icon="🔥" color="orange" />
              <MetricCard label="Success Rate" value={`${(gapesStats.successRate * 100).toFixed(0)}%`} icon="✅" color="green" />
              <MetricCard label="Total Cost" value={`$${gapesStats.cost.toFixed(2)}`} icon="💰" color="yellow" />
            </div>
          )}
        </div>
      )}
          ...
          <div className="bg-gray-800/50 backdrop-blur-xl rounded-2xl p-6 border border-gray-700">
            ...
            {gapesStats && (
              <div className="mt-6 bg-gradient-to-r from-cyan-600/20 to-blue-600/20 border-2 border-cyan-500/50 rounded-xl p-5">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="text-center">
                    <div className="text-3xl font-bold">{gapesStats.totalCalls}</div>
                    <div className="text-xs text-gray-400">Total Calls</div>
                  </div>
                  <div className="text-center">
                    <div className="text-3xl font-bold text-green-400">{gapesStats.streak}</div>
                    <div className="text-xs text-gray-400">Streak</div>
                  </div>
                  <div className="text-center">
                    <div className="text-3xl font-bold text-blue-400">{(gapesStats.successRate * 100).toFixed(0)}%</div>
                    <div className="text-xs text-gray-400">Success Rate</div>
                  </div>
                  <div className="text-center">
                    <div className="text-3xl font-bold text-yellow-400">${gapesStats.cost.toFixed(2)}</div>
